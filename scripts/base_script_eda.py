...
def fstats(df, cols, num_cols):
    ...
    # stats for categorical column leafs where the total unqiue values of leafs < 10
    leaf_count = [df[c].value_counts() for c in list(set(cols) - set(num_cols)) if df[c].unique().size < 10]
...
print fname, preview, cols, num_cols, cat_cols, num_cols_stats, cat_cols_stats, leaf_count
#sys.stdout.flush()
